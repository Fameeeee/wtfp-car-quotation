<template>
  <div class="flex min-h-screen w-screen bg-[#ececec] text-black">
    <AdminSidebar />
    <div class="flex flex-col flex-1 p-6 overflow-hidden h-205" style="margin-left: 100px; margin-right: 20px; margin-top: 45px;">
      ...
      <div class="bg-white p-6 rounded-lg shadow-lg flex-1 overflow-hidden">
        <div class="overflow-x-hidden">
          <table v-if="!loading" class="w-full border-collapse border-spacing-0 table-fixed">
            <thead class="bg-[#f1f5f9] text-[#475569] font-semibold px-5 py-[14px] text-[0.9rem] border-b-2 border-[#e2e8f0] text-left">
              <tr>
                <th class="p-[10px] px-[15px] text-left bg-[#f1f5f9] text-black font-semibold text-[0.9rem] border-b-2 border-gray-300"></th>
                <th class="p-[10px] px-[15px] text-left bg-[#f1f5f9] text-black font-semibold text-[0.9rem] border-b-2 border-gray-300">ไอดี</th>
                <th class="p-[10px] px-[15px] text-left bg-[#f1f5f9] text-black font-semibold text-[0.9rem] border-b-2 border-gray-300">วันที่</th>
                <th class="p-[10px] px-[15px] text-left bg-[#f1f5f9] text-black font-semibold text-[0.9rem] border-b-2 border-gray-300">ชื่อพนักงาน</th>
                <th class="p-[10px] px-[15px] text-left bg-[#f1f5f9] text-black font-semibold text-[0.9rem] border-b-2 border-gray-300">ชื่อลูกค้า</th>
                <th class="p-[10px] px-[15px] text-left bg-[#f1f5f9] text-black font-semibold text-[0.9rem] border-b-2 border-gray-300">รุ่นรถ</th>
              </tr>
            </thead>
            ...
            <tbody>
              ...
              <tr
                v-else
                v-for="history in pagedHistoryList"
                :key="history.id"
...
const filteredHistoryList = computed(() => {
  ...
  return historyList.value.filter((history) => {
    ...
    return historyDate.diff(startDate, "day") >= 0;
  });
});

const pagedHistoryList = computed(() => {
  const arr = filteredHistoryList.value;
  const pages = Math.max(1, Math.ceil(arr.length / itemsPerPage));
