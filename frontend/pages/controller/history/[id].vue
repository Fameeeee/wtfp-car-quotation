    ...
    <div class="content">
      ...
      <div class="history-table">
        ...
        <div class="history-data">
          <table v-if="historyData && historyData.quotations.length" class="history-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>ชื่อลูกค้า</th>
            ...
            <tbody>
              <tr v-for="(quotation, index) in historyData.quotations" :key="index" @click="goToHistory(quotation.id)"
                class="clickable-row">
                <td>{{ quotation.id }}</td>
...
const fetchHistoryData = async () => {
  try {
    const response = await axios.get(`http://localhost:3001/history/${historyId}`);
...
const fetchStaffData = async () => {
  try {
    const staffResponse = await axios.get(`http://localhost:3001/staff/${historyId}`);
