<template>
  <div class="flex min-h-screen bg-[#ececec] w-full text-black">
    <AdminSidebar />
    <div class="content flex flex-col flex-1 ml-[80px]" style="padding: 20px;">
      <div class="header flex justify-between items-center px-5 py-2.5"
        style="padding-inline: 20px; padding-block: 10px;">
        <div class="text-6xl">History</div>
      </div>

      <div class="history-table bg-white p-5 rounded-lg shadow-md relative min-h-[300px] overflow-hidden"
        style="border-collapse: collapse; margin-top: 10px; width: 100%;">
        <div class="back mb-4">
          <button @click="goBack" class="w-24 h-10 bg-red-600 hover:bg-red-800 text-white rounded-md mb-2 cursor-pointer" style="margin-bottom: 10px;">
            < กลับ </button>
        </div>

        <div class="history bg-[#ECECEC] rounded-lg p-2.5 flex gap-2.5">
          <div class="history-img h-[150px] bg-white w-[167px] flex justify-center items-center">
          ...
          <div class="ml-4 bg-[#ECECEC] p-2.5 w-full">
            <div v-if="loading" class="text-gray-500">Loading customer details...</div>

            <div v-else-if="historyData" class="flex flex-col gap-1.5">
              <div class="flex gap-3.5 text-xl font-bold">
                {{ historyData?.customer?.firstName }}
                {{ historyData?.customer?.lastName ?? historyData?.customer?.firstName }}
              </div>

              <div class="text-sm md:text-base">
                สร้างเมื่อ:
                {{ dayjs(historyData?.quotationDate).format('DD/MM/YYYY HH:mm') }}
                · พนักงาน:
                {{ historyData?.staff?.firstName }}
                <span v-if="historyData?.staff?.lastName"> {{ historyData.staff.lastName }}</span>
              </div>
            </div>
        ...
        <div class="history-data space-y-4">
