    ...
    <div class="content">
      ...
      <div class="customer-table">
        ...
        <div class="customer-data">
          <table v-if="customerData && customerData.quotations.length" class="customer-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>ชื่อลูกค้า</th>
            ...
            <tbody>
              <tr v-for="(quotation, index) in customerData.quotations" :key="index" @click="goTocustomer(quotation.id)" class="clickable-row">
                <td>{{ quotation.id }}</td>
...
const fetchStaffData = async () => {
  try {
    const staffResponse = await axios.get(`http://localhost:3001/staff/${customerId}`);
...
const goTocustomer = (quotationId) => {
  router.push(`/controller/customer/${quotationId}`);
};
