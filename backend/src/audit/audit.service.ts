...
export class AuditService {
  ...
  async record(
    ...
    const allowedLevels = ['INFO', 'WARN', 'ERROR', 'FATAL'] as const;
    const safeLevel = allowedLevels.includes(level as any) ? level : 'INFO';
  ...
  async findLatestCreateFrom(
    ...
    try {
      ...
      if (m.from && (m.from.id || m.fromId))
        return Number(m.from.id || m.fromId);
    } catch (e) {}
    return null;
  }
}
